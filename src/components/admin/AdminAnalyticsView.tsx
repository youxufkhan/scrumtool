...
export function AdminAnalyticsView() {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-1.5 bg-slate-100 p-1 rounded-2xl self-start">
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${
              rangePreset === 'week' ? 'bg-white text-indigo-600 shadow-xs' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
            This Week
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${
              rangePreset === 'month' ? 'bg-white text-indigo-600 shadow-xs' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
            This Month
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${
              rangePreset === 'custom' ? 'bg-white text-indigo-600 shadow-xs' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
            Custom Range
          </button>
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs font-semibold text-slate-700">
          <div className="flex items-center gap-1 bg-slate-50 border border-slate-200 rounded-xl px-2.5 py-1.5">
            ...
            <input
              ...
              className="bg-transparent focus:outline-hidden text-slate-800 font-semibold"
            />
          </div>
          ...
          <button
            ...
            className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold text-xs flex items-center gap-1.5 shadow-xs transition-all cursor-pointer"
          >
      ...
      {loading ? (
        <div className="text-center py-20 text-slate-400 text-sm">Computing analytics metrics...</div>
      ) : !analytics ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-xs flex items-center justify-between">
              <div>
                <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Total Team Hours</span>
                <div className="flex items-baseline gap-1 mt-1">
                  <span className="text-3xl font-black text-indigo-600">{analytics.totalHours.toFixed(1)}</span>
            ...
            <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-xs flex items-center justify-between">
              <div>
                <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Planned Work Ratio</span>
                <div className="flex items-baseline gap-1 mt-1">
                  <span className="text-3xl font-black text-emerald-600">{analytics.totalPlannedHours.toFixed(1)}</span>
                  <span className="text-xs font-semibold text-slate-400">hrs</span>
                </div>
              </div>
            ...
            <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-xs flex items-center justify-between">
              <div>
                <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Ad-hoc / Scope Creep</span>
                <div className="flex items-baseline gap-1 mt-1">
                  <span className="text-3xl font-black text-purple-600">{analytics.totalAdHocHours.toFixed(1)}</span>
                  <span className="text-xs font-semibold text-slate-400">hrs ({adHocPercentage}%)</span>
                </div>
              </div>
                ...
                {analytics.memberBreakdown.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={analytics.memberBreakdown} margin={{ top: 10, right: 10, left: -20, bottom: 20 }}>
                      ...
                      <XAxis dataKey="memberName" tick={{ fontSize: 11 }} interval={0} angle={-15} textAnchor="end" />
                  ...
                  <div className="h-full flex items-center justify-center text-xs text-slate-400">No member data</div>
                )}
              </div>
            </div>
            ...
            <div className="bg-white p-5 sm:p-6 rounded-3xl border border-slate-200 shadow-xs">
              ...
              <div className="h-64 w-full">
                {analytics.projectBreakdown.length > 0 && analytics.totalHours > 0 ? (
                  ...
                  <div className="h-full flex items-center justify-center text-xs text-slate-400">No project data</div>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-5 sm:p-6 rounded-3xl border border-slate-200 shadow-xs">
            ...
            <div className="h-64 w-full">
              {analytics.dailyTrends.length > 0 ? (
                ...
                <div className="h-full flex items-center justify-center text-xs text-slate-400">No daily data</div>
              )}
            </div>
          </div>
                ...
                <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
                  {analytics.memberBreakdown.map((m) => (
                    <tr key={m.memberId} className="hover:bg-slate-50/70 transition-colors">
                      <td className="px-5 py-3.5 font-bold text-slate-900">{m.memberName}</td>
                      <td className="px-5 py-3.5 text-indigo-600 font-black">{m.totalHours.toFixed(1)} hrs</td>
