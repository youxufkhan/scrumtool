    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 sm:p-5 rounded-3xl border border-slate-200 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2 px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-900">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex-1 md:flex-none px-4 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold flex items-center justify-center gap-2 shadow-xs transition-all cursor-pointer"
          >
            {copiedSlack ? (
              <>
      ...
      {actionMessage && (
        <div className="p-3 bg-indigo-50 border border-indigo-200 text-indigo-800 text-xs font-semibold rounded-2xl">
          {actionMessage}
        </div>
      )}
      ...
      {report && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
          <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs">
            <span className="text-xs font-semibold text-slate-500 block">Total Team Hours</span>
            <div className="flex items-baseline gap-1 mt-1">
              <span className="text-2xl font-black text-indigo-600">{report.totalTeamHours.toFixed(1)}</span>
              <span className="text-xs font-semibold text-slate-400">hrs</span>
            </div>
          </div>

          <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs">
            <span className="text-xs font-semibold text-slate-500 block">Standup Submitted</span>
            <div className="flex items-baseline gap-1 mt-1">
          ...
          <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs">
            <span className="text-xs font-semibold text-slate-500 block">Blocked Impeding Tasks</span>
            <div className="flex items-baseline gap-1 mt-1">
              <span className={`text-2xl font-black ${report.blockedTasksCount > 0 ? 'text-amber-600' : 'text-slate-700'}`}>
          ...
          <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs">
            <span className="text-xs font-semibold text-slate-500 block">Team Status</span>
            <div className="mt-1">
              {report.submittedMembersCount === report.totalMembersCount ? (
                <span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded-lg inline-block">
                  ✓ 100% Complete
                </span>
              ) : (
                <span className="text-xs font-bold text-amber-600 bg-amber-50 px-2 py-1 rounded-lg inline-block">
                  ⏳ In Progress
                </span>
              )}
            </div>
      ...
      {loading ? (
        <div className="text-center py-16 text-slate-400 text-sm">Loading team standup board...</div>
              ...
              >
                <div>
                  ...
                  <div className="flex items-start justify-between gap-3 pb-3 border-b border-slate-100 mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      {isOnLeave ? (
                        <span className="text-[11px] font-bold px-2 py-0.5 rounded-lg bg-emerald-50 text-emerald-700 border border-emerald-100 flex items-center gap-1">
                        ...
                        <span className="text-[11px] font-bold px-2 py-0.5 rounded-lg bg-amber-50 text-amber-800 border border-amber-200 flex items-center gap-1">
                        ...
                        >
                          <div className="flex items-center justify-between gap-2 mb-1">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              ...
                              {task.is_ad_hoc && (
                                <span className="text-[9px] font-bold uppercase px-1.5 py-0.2 rounded bg-purple-100 text-purple-700">
                                  Ad-hoc
                                </span>
                              )}
                              {task.status === 'done' ? (
                                <span className="text-[9px] font-bold text-emerald-700">✓ Done</span>
                              ) : task.status === 'blocked' ? (
                                <span className="text-[9px] font-bold text-amber-700">⚠️ Blocked</span>
                              ) : (
                                <span className="text-[9px] font-medium text-slate-500">🔄 In Progress</span>
                              )}
                            </div>
                          ...
                          {task.blocker_note && (
                            <p className="text-[11px] text-amber-800 font-semibold mt-1 bg-amber-50 p-1.5 rounded border border-amber-200">
                ...
                {isLocked && !isOnLeave && (
                  <div className="mt-4 pt-3 border-t border-slate-100 flex justify-end">
                    <button
                      ...
                      className="text-[11px] font-semibold text-slate-500 hover:text-indigo-600 flex items-center gap-1 p-1 hover:bg-indigo-50 rounded transition-colors"
