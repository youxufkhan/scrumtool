...
describe('Roadmap Actions', () => {
  ...
  it('handles Supabase client branch correctly when Supabase is configured', async () => {
          ...
          return {
            ...
            select: vi.fn(() => ({
              eq: vi.fn((col1: string, val1: any) => ({
                order: vi.fn(() => Promise.resolve({ data: [fakeRow], error: null })),
                eq: vi.fn((col2: string, val2: any) => ({
                  maybeSingle: vi.fn(() => Promise.resolve({ data: fakeRow, error: null })),
                })),
              })),
            })),
          };
    ...
    // Test save
    const saveRes = await saveMemberRoadmap({
      member_id: 'member-1',
      title: 'DB Roadmap',
      tech_skills_score: 5,
      soft_skills_score: 4,
    ...
    // Test get history
    const history = await getMemberRoadmapHistory('member-1');
    expect(history).toEqual([fakeRow]);

    // Test get active
    const active = await getActiveRoadmap('member-1');
    expect(active).toEqual(fakeRow);
